...
export const ChatInput: React.FC = () => {
  ...
  return (
    <div className="px-4 pb-4 pt-1">
      <div className="bg-[#F8F9FA] rounded-[1.2rem] p-3.5 border border-[#E9ECEF] focus-within:bg-white focus-within:border-[#0066FF]/20 focus-within:shadow-[0_2px_8px_rgba(0,102,255,0.05)] transition-all duration-300">
        <textarea
          value={input}
          onChange={(e) => presenter.chatManager.setInput(e.target.value)}
          onKeyDown={(e) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); }}}
          placeholder="输入创作指令，如：‘生成一个赛博朋克风格的分镜图’"
          className="w-full bg-transparent px-1.5 text-[14px] focus:outline-none resize-none h-14 placeholder:text-[#ADB5BD] text-black font-medium leading-relaxed"
        />
        ...
        <div className="flex items-center justify-between mt-2.5 px-0.5">
          <div className="flex items-center gap-1">
             ...
             <Tooltip content="AI 创意润色" position="top">
               <button 
                 className="w-8 h-8 rounded-xl bg-[#E3F2FD] text-[#0066FF] flex items-center justify-center transition-all hover:bg-[#0066FF] hover:text-white active:scale-90"
               >
          ...
          <div className="flex items-center">
             <Tooltip content="发送指令 (Enter)" position="top">
               ...
               >
                 {isTyping ? (
                   <i className="fa-solid fa-circle-notch animate-spin text-[13px]"></i>
                 ) : (
